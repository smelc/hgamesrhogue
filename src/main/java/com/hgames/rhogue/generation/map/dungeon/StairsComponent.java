...
import com.hgames.rhogue.generation.map.dungeon.connection.IConnectionFinder;
import com.hgames.rhogue.generation.map.dungeon.stair.IStairGenerator;
import com.hgames.rhogue.generation.map.dungeon.stair.StairGenerator;
...
public class StairsComponent implements GeneratorComponent {
	...
	protected IStairGenerator getStairGenerator(DungeonGenerator gen, GenerationData gdata,
		...
		final StairGenerator result = new StairGenerator(gen.logger, gen.rng, gen, dungeon, objective, upOrDown, gdata, connections);
		result.setStairValidDirection(gen.stairUpValidDirection, true);
		result.setStairValidDirection(gen.stairDownValidDirection, false);
		return result;
	}
