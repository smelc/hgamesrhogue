...
public interface Rectangle extends Zone {
	...
	class Utils {
		 ...
		 *            a Rectangle
		 * @param c
		 *            a Coord to check against r for presence
		 ...
		 *            a Rectangle
		 * @param x
		 *            x-coordinate of a point to check against r
		 * @param y
		 *            y-coordinate of a point to check against r
		 * @return Whether {@code r} contains {@code c}.
		 */
		 ...
		 *            a Rectangle
		 * @param cs
		 *            a Collection of Coord to check against r; returns true if r
		...
		/**
		 * @param rs
		 *            an Iterable of Rectangle items to check against c
		 * @param c
		 *            a Coord to try to find in any of the Rectangles in rs
		 ...
		 * @param r
		 * @param buf
		 *            An array of (at least) size 4, to hold the 4 corners. It is
		 ...
		 * @param cardinal
		 * @param buf
		 *            The buffer to fill or {@code null} to let this method allocate.
		 ...
		 */
		public static List<Coord> getBorder(Rectangle r, Direction cardinal, /* @Nullable */ List<Coord> buf) {
			Coord start = null;
			Direction dir = null;
			int len = -1;
			switch (cardinal) {
	...
	class Impl extends Zone.Skeleton implements Rectangle {
		...
		public Zone shrink() {
			if (width <= 2 || height <= 2)
				return new ListZone(Collections.<Coord>emptyList());
			else
				return new Rectangle.Impl(bottomLeft.translate(Direction.UP_RIGHT), width - 2, height - 2);
		}
